...
from aiogram.fsm.storage.memory import MemoryStorage  # fallback only

# Persistent FSM storage — survives restarts
try:
    from aiogram.fsm.storage.redis import RedisStorage as _RedisStorage
    _REDIS_URL = os.getenv("REDIS_URL", "")
    if _REDIS_URL:
        _persistent_storage = _RedisStorage.from_url(_REDIS_URL)
    else:
        raise ImportError("no REDIS_URL")
except Exception:
    try:
        from aiogram_sqlite_storage import SQLStorage as _SQLStorage  # type: ignore
        _persistent_storage = _SQLStorage("fsm_states.db")
    except ImportError:
        import logging as _log
        _log.getLogger(__name__).warning(
            "⚠️  Используется MemoryStorage — FSM-состояния сбросятся при рестарте. "
            "Установите aiogram-sqlite-storage или укажите REDIS_URL."
        )
        _persistent_storage = MemoryStorage()
...
BOT_TOKEN             = _env_str("ARION_TOKEN", "")
MAIN_ADMIN_ID         = _env_int("MAIN_ADMIN_ID", 0)
CONSIDERATION_CHAT_ID = _env_int("CONSIDERATION_CHAT_ID", 0)
REPORT_CHAT_ID        = _env_int("REPORT_CHAT_ID", 0)
CONS_GROUP_CHAT_ID    = _env_int("CONS_GROUP_CHAT_ID", 0)
APPEAL_CHAT_ID        = _env_int("APPEAL_CHAT_ID", 0)
API_PORT              = _env_int("API_PORT", 8080)
API_SECRET            = _env_str("API_SECRET", "CHANGE_ME_SECRET_KEY")
MINI_APP_URL          = _env_str("MINI_APP_URL", "https://your-username.github.io/arion-miniapp")
...
def fsm_schedule(user_id: int, state: FSMContext) -> None:
    ...
    _fsm_timers[user_id] = loop.call_later(FSM_TIMEOUT_SEC, _schedule_clear)
    ...
    try:
        async with mdb() as db:
            await db.execute(
                ...
                "stat_value = CAST(stat_value AS INTEGER) + excluded.stat_value, "
                "updated_at = excluded.updated_at",
                (key, str(delta), _now_str()),
            )
            await db.commit()
    except Exception as exc:
...
async def main() -> None:
    ...
    _api_task = asyncio.create_task(start_api_server())

    def _on_api_task_done(t: asyncio.Task) -> None:
        if not t.cancelled() and (exc := t.exception()):
            log.critical("❌ HTTP API упал: %s", exc, exc_info=exc)

    _api_task.add_done_callback(_on_api_task_done)

    bot = Bot(token=BOT_TOKEN)
    dp  = Dispatcher(storage=_persistent_storage)
